...
- (Optional) HTML pages generated from the Canvas API:
    ...
    - Discussions
    - Modules
- (Optional) Markdown versions of HTML/Word/PowerPoint/Excel/EPUB/… files
...
- [pandoc](https://pandoc.org/installing.html) for HTML/Word/PowerPoint/Excel/EPUB → Markdown conversion. Use the full official release: several distribution packages omit the PowerPoint/Excel readers. The Docker `full` image bundles the official full build.
...
| `--notebooklm`          | Upload recent courses to NotebookLM.               | Enabled with `NOTEBOOKLM_AUTH_JSON`/`NOTEBOOKLM_PROFILE` |
...
Every HTML/Word/PowerPoint/Excel/EPUB/text file in the export is converted to Markdown next to the original (`.md`), which makes the data easy to search and ready for tools such as NotebookLM that do not accept HTML.
...
- HTML pages generated by this tool go through the same pandoc pipeline; the Lua filter also unwraps the exporter's own wrappers (`main`, `comment`, `reply`, ...) so the Markdown stays clean, and constructs Markdown cannot express are embedded as HTML. Generated `.md` files carry a marker so NotebookLM uploads never duplicate their HTML source.
- Files that pandoc cannot read (legacy `.doc`, …) are left as they are.
...
2. Copy the resulting cookies to the machine/container that runs the exporter:
   ...
   ```
   `storage_state.json` is a bearer credential for your Google account: keep it private and never commit it. In the container, pass `-e NOTEBOOKLM_AUTH_JSON`. Alternatively, mount the profile directory (`-v ~/.notebooklm:/home/exporter/.notebooklm:ro`) and set `NOTEBOOKLM_PROFILE=<name>`; uploads start automatically when either variable is set.

When `NOTEBOOKLM_AUTH_JSON` (or `NOTEBOOKLM_PROFILE`) is set, the upload stage runs automatically after each course is exported (disable with `--no-notebooklm`, force with `--notebooklm`).
